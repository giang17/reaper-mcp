...
ReaperMCP exposes 172 tools by default. Smaller or cheaper LLMs (Groq Llama 3 caps at 128, Claude Haiku and some local models lower still) will silently truncate. Set `REAPER_MCP_PROFILE` in your client's `env` block to register only a workflow-specific subset:

| Profile | Tools | For |
|---------|------:|-----|
| `full` *(default)* | ~172 | Claude, GPT-4, Gemini-class models |
| `composition` | ~129 | Writing / editing music (incl. full chop pipeline, batch item/marker edits, ReaScript) |
